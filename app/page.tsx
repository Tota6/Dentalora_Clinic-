...
function Logo({ className = '' }: { className?: string }) {
  return <img src="/dentalora-logo-v2.svg" className={className} alt="Dentalora Dental Clinic" />;
}
...
export default function Home() {
  ...
  return (
    <main dir={ar ? 'rtl' : 'ltr'} className={styles.page}>
      ...
      <section id="services" className={styles.services}><div className={styles.sectionHead}><div><div className={styles.eyebrow}>{ar ? 'خدماتنا' : 'OUR SERVICES'}</div><h2>{ar ? 'رعاية أسنان متكاملة لك ولعائلتك' : 'Comprehensive Dental Care for You & Your Family'}</h2></div><a href="#booking">{ar ? 'احجزي الآن' : 'Book Now'} →</a></div><div className={styles.serviceGrid}>{services.map(([num,title,text]) => <article className={styles.serviceCard} key={title}><div className={styles.serviceIcon}>⌁</div><span>{num}</span><h3>{title}</h3><p>{text}</p><a href="#booking">{ar ? 'اعرفي المزيد' : 'Learn More'} →</a></article>)}</div></section>

      <section id="about" className={styles.locationSection}><div className={styles.locationPanel}><div className={styles.eyebrow}>{ar ? 'موقعنا' : 'OUR LOCATION'}</div><h2>{ar ? 'زورينا' : 'Visit Us'}</h2><p>{clinicAddress}</p><div className={styles.phoneRow}>{phones.map(phone => <a key={phone} href={`tel:${phone}`}>{phone}</a>)}</div><a className={styles.directionButton} href={mapUrl} target="_blank" rel="noreferrer">{ar ? 'افتحي الاتجاهات' : 'Get Directions'} →</a></div><a href={mapUrl} target="_blank" rel="noreferrer" className={styles.mapVisual} aria-label="Open Dentalora location in Google Maps"><div className={styles.mapGrid}/><div className={styles.mapPin}>● <strong>Dentalora Clinic</strong></div><span className={styles.mapLabel}>Dokki · Giza · Egypt</span></a></section>

      <section id="contact" className={styles.contactStrip}><div><span className={styles.stripIcon}>⌖</span><b>{ar ? 'الموقع' : 'Location'}</b><span>39 El-Batal Ahmed Abd El-Aziz, Dokki, Giza</span></div><a href="mailto:dentaloradentalclinic@gmail.com"><span className={styles.stripIcon}>✉</span><b>{ar ? 'البريد الإلكتروني' : 'Email Us'}</b><span>dentaloradentalclinic@gmail.com</span></a><div><b>{ar ? 'تابعينا' : 'Follow Us'}</b><div className={styles.socials}>{socials.map(s => <a key={s.key} href={s.href} target="_blank" rel="noreferrer" aria-label={s.label}><SocialIcon type={s.key}/></a>)}</div></div><div className={styles.qr}><img src={qrImage} alt="Dentalora QR Code"/><span>{ar ? 'امسحي للتواصل' : 'Scan to Connect'}</span></div></section>
